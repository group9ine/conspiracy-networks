logbins <- function(data, base = 10, dlog = 0.1) {
  ...
  x <- seq(0, log(max(values), base) + dlog, by = dlog)
  lower <- ceiling(base^(x - 0.5 * dlog))
  upper <- floor(base^(x + 0.5 * dlog))
  ...
  y <- c(
    counts[vals < lower[1]],  # these are from counts
    # here we average the counts falling inside each remaining bin
    mapply(
      \(l, u) sum(counts[l <= vals & vals <= u]) / (1 + u - l),
      lower, upper
    )
  )
  y <- log(y, base)
